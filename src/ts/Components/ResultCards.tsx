            ...
            GlobalItems?.typeOfDeck === "kanji" ?
              ...
              GlobalItems?.typeOfDeck === "katakana" || GlobalItems?.typeOfDeck === "hiragana" ?
                card ?
                  card.map((c) => (
                    <div key={c.id} className=" font-bold  border-[3px] border-[#0096C7] dark:border-[#0bb744] p-[2rem] bg-white dark:bg-[#1C1C21] dark:text-white rounded-[10px] w-[300px] sm:basis-[80%] lg:basis-[24%] gap-3">
                      <p className="mx-auto my-[0.5rem] p-2 bg-[#0096C7] dark:bg-[#0BB744] dark:text-black text-white rounded-md w-[80%] text-center ">{c[GlobalItems.typeOfDeck]}</p>
                      <p>English: {c.english}</p>
                    </div>
                  )) : null : null
          }
        </div>
